...
class Index extends Page
  className: "#{Page::className} hackday"
  template: require('views/hackday/index')
      ...
      when 'subjects'
        Api.get "#{ @url() }/subjects?page=#{ @subjectsPage += 1 }&per_page=12", (results) =>
          if results.length > 0
            ...
            @subjectList.append require('views/hackday/subjects')(subjectPage)
      ...
      when 'groups'
        Api.get "#{ @url() }/groups?page=#{ @groupsPage += 1 }&per_page=10", (results) =>
          if results.length > 0
            ...
            @groupList.append require('views/hackday/groups')(groupPage)
      ...
      when 'collections'
        Api.get "#{ @url() }/collections?page=#{ @collectionsPage += 1 }&per_page=10", (results) =>
          if results.length > 0
            ...
            @collectionList.append require('views/hackday/collections')(collectionPage)
          
          if results.length < 10
            target.attr disabled: true

class Hackday extends SubStack
  controllers:
    index: Index
  ...
  default: 'index'
  className: 'stack hackday'


module.exports = Hackday
